...
class Palette {
  ...
  color colDark(float v, float per) {    // v [0.0;1.0]
    return color(r.v(v) * per, g.v(v) * per, b.v(v) * per);
  }
...
color darken(color col, float per) {
  return color(red(col) * per, green(col) * per, blue(col) * per);
}
